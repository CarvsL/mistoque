...
export default function CodeTwo() {
  ...
  const reenviarCodigo = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/reenviar-codigo-senha`, {
        method: "POST",
        headers: {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/verify-code-senha`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      navigate("/resenha", {
        state: {
          celular: location.state?.celular,
          token: data.token // Token de segurança para redefinir a senha
        }
      });
      ...
      <div className={styles.fundo}>
        <div className={styles.loginContainer}>
          ...
          <p className={styles.loginSubtitulo}>
            Um código de verificação foi enviado para {location.state?.celular}
          </p>
